...
const SearchBar = () => {
    ...
    return (
        <>
            <div
                className={'flex-1 flexCenter dark:bg-nft-black-2 bg-white border dark:border-nft-black-2 border-nft-gray-2 py-3 px-4 rounded-md'}>
                <Image className={theme === 'light' && 'filter invert'} src={images.search} alt="search"
                       objectFit={'contain'} width={20} height={20}/>
            ...
            <div onClick={() => setToggle((prevState) => !prevState)}
                ...
                {toggle && (
                    <div className={'absolute top-full left-0 right-0 w-full mt-3 z-10 dark:bg-nft-black-2 bg-white border dark:border-nft-black-2 border-nft-gray-2 py-3 px-4 rounded-md'}>
                        {['Recently added', 'Price (low to high)', 'Price (high to low)'].map((item, index) => (
                            <p className={'font-poppins dark:text-white text-nft-black-1 font-normal text-xs my-3 cursor-pointer'} key={index}>{item}</p>
                        ))}</div>
                )}
            </div>
